...
export default function WhatsAppButton({
  ...
  const sendReminder = async (e: MouseEvent<HTMLAnchorElement>) => {
    ...
    setReason('');
    try {
      await sendViaWorker({ base: workerBase, token: workerToken, to: number, text: message });
      setSend('sent');
      setTimeout(() => setSend('idle'), 2500);
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Fallo del worker.';
      setSend('error');
      setReason(msg);
      setTimeout(() => {
        setSend('idle');
